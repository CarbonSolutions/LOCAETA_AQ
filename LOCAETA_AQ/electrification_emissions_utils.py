...
class ElectrificationEmissionProcessor:
    ...
    def reformat_powerplant(self, df):
        ...
        logger.info(f'before :, {total_before}, after: , {total_after}') 
    ...
    def mapping_powerplant_to_nei(self, nei_with_powerplant, nei_all_pt, unique_identifier_columns, is_base):
        ...
        for nei_col, Facilities_col in pollutant_map.items():
            ...
            logger.info(f"{nei_col}, {Facilities_col}")
            ...
            logger.info(f"{nei_col}: # fallback allocations due to zero NEI = {mask_zero_total.sum()}")
            ...
            logger.info(f"after {Facilities_col} splitting : , {nei_with_powerplant[f'{nei_col}'].sum()}")
    ...
    def diagnose_egrid_nei_mismatch(self, egrid, nei_all_pt, nei_with_powerplant):
        ...
        """
        logger.info("=== eGRID-NEI EIS_ID MISMATCH DIAGNOSTIC ===")
        ...
        logger.info(f"Unique EIS_IDs in eGRID: {len(egrid_eids)}")
        logger.info(f"Unique EIS_IDs in NEI: {len(nei_eids)}")
        logger.info(f"Unique EIS_IDs after merge: {len(merged_eids)}")
        ...
        logger.info(f"EIS_IDs in NEI but not in eGRID: {len(nei_not_in_egrid)}")
        logger.info(f"EIS_IDs from eGRID lost after merge: {len(egrid_lost_in_merge)}")
        ...
        if len(egrid_not_in_nei) > 0:
            ...
            for idx, row in sample_missing.iterrows():
                logger.info(f"  EIS_ID {row['EIS_ID']}: NOx={row['NOx_tons_base']}, PM2.5={row['PM2.5_tons_base']}")
        ...
        if len(egrid_lost_in_merge) > len(egrid_not_in_nei):
            ...
            logger.info(f"Extra lost EIS_IDs: {len(extra_lost)}")
            logger.info(f"Sample extra lost: {list(extra_lost)[:5]}")
        ...
        logger.info(f"\n=== EMISSION ACCOUNTING ===")
        logger.info("Original eGRID sums:")
    ...
    def check_eis_id_formats(self, egrid, nei_all_pt):
        ...
        logger.info("=== EIS_ID FORMAT CHECK ===")
        
        egrid_sample = egrid['EIS_ID'].head(10).tolist()
        nei_sample = nei_all_pt['EIS_ID'].head(10).tolist()
        
        logger.info(f"Sample eGRID EIS_IDs:, {egrid_sample}")
        logger.info(f"Sample NEI EIS_IDs:, {nei_sample}")
        
        # Check data types
        logger.info(f"\neGRID EIS_ID dtype: {egrid['EIS_ID'].dtype}")
        logger.info(f"NEI EIS_ID dtype: {nei_all_pt['EIS_ID'].dtype}")
        ...
        if egrid_spaces or nei_spaces:
            logger.info("❌ Found leading/trailing spaces in EIS_IDs")
            logger.info(f"  eGRID has spaces: {egrid_spaces}")
            logger.info(f"  NEI has spaces: {nei_spaces}")
        else:
            logger.info("✅ No leading/trailing spaces found")
        ...
        logger.info(f"NEI EIS_ID lengths: {sorted(nei_lengths)}")
    ...
    def mapping_non_powerplant_to_nei(self, nei_with_non_powerplant, nei_df, unique_identifier_columns, is_base):
        ...
        for nei_col, powerplant_col in pollutant_map.items():
            logger.info(f" mapping non powerpo=lant to nei: {nei_col}, {powerplant_col}")
    ...
    def process_powerplant_scenario(self, scen_name, emis_name, nei_all_pt, overall_scenario):
        ...
        if unique_id_cols:
            logger.info(f"Columns that uniquely identify rows:, {unique_id_cols}")
        else:
            logger.info("No combination of columns uniquely identifies rows.")

        # Merge NEI + eGRID
        nei_with_pp = nei_with_pp.merge(egrid, on="EIS_ID", how="left")
        true_sum = nei_with_pp.groupby('EIS_ID')[['PM2.5_tons_base', 'NH3_tons_base', 
                                                  'VOC_tons_base', 'NOx_tons_base', 'SO2_tons_base']].first().sum()
        logger.info(f"intial nei_with_powerplant sum, {true_sum}")
        ...
        for is_base in [True, False]:
            ...
            logger.info(f"after filling sum:\n, {mapped_df[self.NEI_cols].sum()}")
            logger.info(f"final size:, {mapped_df.shape}, {unmapped_df.shape}")
            ...
            if not mapped_df.empty:
                ...
                logger.info(f"Saved mapped data to {save_filename}")

        return egrid, mapped_df, unmapped_df


    # process other facilities emissions
    def process_non_powerplant(self,  scen_emis_list, unmapped_df, nei_all_pt, overall_scenario):
        ...
        if duplicates.empty:
            logger.info("Great, no duplicate in CCS emissions")
        else:
        ...
        if len(common_ids) == 0:
            logger.info("Great, no duplicated EIS_ID between cs_emis and egrid")
    # use the unmapped nei dataframe for cs_emis emission processing 
            nei_df = unmapped_df.copy()
        else:
            logger.info(f"Duplicated EIS_ID between CCS and eGRID:, {common_ids}")
        ...
        logger.info(f"cs_emis base sum:, {cs_emis.filter(like='_tons_base').sum()}")
        logger.info(f"cs_emis final sum:, {cs_emis.filter(like='_tons_final').sum()}")
    ...
    def create_non_powerplant_symlinks(self, scen_emis_list, overall_scenario):
        """
    ...
    def compare_emissions(self, scen_emis_list, overall_scenario):
        ...
        output_dir = os.path.join(
            self.config['output']['plots_dir'], # overall_scenario,
        ...
        logger.info(f"checking plot bar legend names: {new_cols}")
    ...
    def compare_with_original(self, scen_emis_list, overall_scenario):
