...
class NEIEmissionProcessor:
    ...
    def filter_and_delete_keys(self, files_dict):
        ...
        # List of keys to skip (canada and mexico emissions are excluded)
        ...
        for key in list(files_dict.keys()):
            ...
            for file in files_dict[key]:
                if not any(substring in file for substring in substrings_to_check):
                    ...
                    logger.info(f"Excluding file due to match: {file}")
    ...
    def reproject_and_save_gdf(self, gdf):
        # Check if the GeoDataFrame is already in the target CRS
        current_crs = gdf.crs
        target_proj = CRS.from_string(self.config['input']['target_crs'])
        
        if current_crs != target_proj:
            logger.info(f"Reprojecting from {current_crs} to {target_proj}")
            gdf = gdf.to_crs(self.config['input']['target_crs'])
        else:
            logger.info(f"GeoDataFrame is already in the target CRS: {target_proj}")
        return gdf
    ...
    # Function to save each state total sum for debugging purpose 
    def save_state_emis(self, file_name, gdf):
        ...
        output_state_emis_csv = self.config['output']['output_dir'] +  self.config['output']['state_emissions_sum']
    ...
    # Main function to read and process a single NEI file
    def process_nei_file(self, file_path):

        emis_df = pd.read_csv(file_path, comment='#', dtype={'region_cd': str})

        logger.info(f"Inside process_nei_file: {file_path}, {emis_df.columns}")
    ...
    def combined_point_sources(self):

        # List of shapefiles to combined
        ...
        for f in files:
            file_path = os.path.join(self.config['output']['output_dir'], f)
            gdf = gpd.read_file(file_path)
            
            # Add a column with source sector information, which is the file name (without '.shp')
    ...
    def netcdf_to_shapefile(self, gdf_fips):

        # Define the input NetCDF file and the output shapefile path
        netcdf_path = Path(self.config['input']['netcdf_dir'])
        output_path = self.config['output']['output_dir']

        allowed_filenames = self.config['input'].get('netcdf_filenames', [])
        netcdf_files = []
        # get the list of netcdf files to process
        for f in netcdf_path.iterdir():
            if f.is_file() and f.suffix == ".ncf":

                #!!!! Warning - Hardcoded source file name!!! 
                #if "cb6_20k_onroad" in f.name or "rail" in f.name:
                if any(name in f.name for name in allowed_filenames):
                    logger.warning(
                    f"These files are matched for netcdf processing: {f.name}. ")
                    netcdf_files.append(f)  # .stem removes ".ncf"

        for netcdf_file in netcdf_files:

            logger.info(f"processing into shapefile for {netcdf_file}")
            ...
            for variable_name in ds.data_vars:
                ...
                if variable_name in ['VOC', 'VOC_INV', 'XYL', 'TOL', 'TERP', 'PAR', 'OLE', 'NVOL', 'MEOH', 
                    ...
                    logger.info(f"checking variable for PM2_5, {variable_name}, {data}")
            ...
            target_proj = Proj(self.config['input']['target_crs'])
            ...
            logger.info(f'Shapefile saved to {output_shapefile}')
