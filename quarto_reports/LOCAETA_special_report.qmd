---
title: "LOCAETA report: Data Center in Memphis, TN with clean CCS"
format:
  html:
...
# set run_name
run_list = ['TN_DataCenter_NOx_2ppm_clean_CCS', 'TN_DataCenter_NOx_2ppm'] 

run_name = run_list[0] # ["current_2020","decarb95_2050"] # ,"highREcost_2050"] # ['TN_DataCenter_NOx_2ppm', 'TN_DataCenter_NOx_25ppm'] # ['LA_CCS', 'LA_CCS_noNH3']
...
# Introduction 
The primary objectives of this study are to assess the socioeconomic and air quality impacts of energy demand by single hypothetical data center in Memphis, TN, which is assumed to be powered by on-site fossil fuels. 

# Emission Changes by Key Pollutants

We estimated changes in air pollutant emissions resulting from running fossil fuels-based generators in data center in Memphis, TN: exact location is at latitude = 35.0598925 and longitude = -90.1559922684821. Two scenarios are considered in this study: emissions with assuming NOx 2ppm and the same but with clean CCS tech (i.e., CCS tech without NH3 and VOC emission increase).   

@tbl-total-emissions-changes summarizes the total emissions by pollutants, resulting from the data center in Memphis, TN. These emissions are not available in the NEI2020 emissions inventory. Since there is no established EIS ID and SCC for this emission, we use "placeholder" EIS ID and SCC values. 

```{python}
#| label: tbl-total-emissions-changes
#| tbl-cap: Air quality emissions from the data center in Memphis, TN 
#| tbl-colwidths: [20,60]

import pandas as pd

df = pd.read_csv(f'{emis_output_path}/Total_emissions.csv')
