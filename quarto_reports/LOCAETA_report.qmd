...
# set run_name
run_name = 'USA_CCS'
run_list = ['USA_CCS', 'USA_CCS_wo_NH3_VOC', 'USA_zero_out_CCS']
...
#   '/Users/yunhalee/Documents/LOCAETA/CS_emissions/USA_CCS.shp')
...
@fig-total-emis-diff shows total emissions by species at all industrial facilities, both without CCS (shown as "NEI" in the figure legend) and with CCS (shown as "USA_CCS"), as well as where emissions are entirely zeroed out at facilities with CCS implementation (shown as "USA_zero_out_CCS"). The zero_out case is included to provide a reference for the relative impact of CCS deployment. 

```{python}
#| label: fig-total-emis-diff
#| fig-cap: "NEI vs CCS vs zero_out emissions at all industrial point source facilities (i.e., ptegu, pt_oilgas, and ptnonipm)"

from IPython.display import display, HTML
img_path = f"{emis_output_path}/NEI_vs_CCS_vs_zero_out_emissions_all_industrial_facilities.png"
html = f'<figure><img src="{img_path}" alt="Total Emissions" class="center" width="800"></figure>'
display(HTML(html))
...
for i, pollutant in enumerate(pollutants):
    total_original = totals[f'{pollutant}_nei']
