...
params:
  ...
  run_list: ["current_2020", "current_2020_PJM_East", "current_2020_CAISO"]
  plots_dir: /Users/yunhalee/Documents/LOCAETA/LOCAETA_AQ/outputs/report/current_2020
...
report_root = cfg['base_dirs']['report_root']
output_root = cfg['base_dirs']['output_root']
qmd_template = config["input"]["qmd_template"]
plots_dir =  os.path.join(config["output"]["plots_dir"], report_run) 
qmd_path = os.path.join(report_root, qmd_template)

# imap output 
inmap_output_dir = os.path.join(output_root, 'inmap')

# gather all run names 
...
run_list = output_pairs.keys()

# Convert run_list to a proper list if it's dict_keys
run_list = list(run_list) if not isinstance(run_list, list) else run_list
...
if scenario == "datacenter_emissions": 
  scen_name = "Data Center scenario"
elif scenario == "ccs_emissions": 
  scen_name = "Carbon Capture and Storage scenario"
elif scenario == "electrification_emissions": 
  scen_name = "Electrification scenario"
else:
  raise ValueError
...
<div style="
  ...
  border-radius:10px;
  margin:20px 0;">
  <h1 style='margin:0; font-size:2.2em;'>{scen_name} Report</h1>
  <h3 style='margin:5px 0 0;'>Run: {report_run}</h3>
</div>
"""))
```
...
with open(f"{plots_dir}/intro.txt", "r") as f:
    intro_text = f.read()
...
```{python}
with open(f"{plots_dir}/emis_description.txt", "r") as f:
    emis_text = f.read()
...
#| fig-cap: Overall emissions 

import matplotlib.pyplot as plt
import matplotlib.image as img


img_path =f'{plots_dir}/overall_emis_plot.png'
...
df = pd.read_csv(f'{plots_dir}/simulations_description.csv')
...
img_path = f"{plots_dir}/area_weighted_averages_all_runs.png"
image = img.imread(img_path)
...
file_path = f"{plots_dir}/area_weighted_averages_all_runs.csv"
...
for ax, run in zip(axes, run_list):
    img_path = f"{inmap_output_dir}/{run}/TotalPM25_Percent Change_with_basemap.png"
    image = img.imread(img_path)
...
img_path = f"{plots_dir}/Annual Health Benefits.png"
image = img.imread(img_path)
...
file_path = f"{plots_dir}/Health_Benefit.csv"
...
img_path = f"{plots_dir}/Normalized Health Benefits (per million).png"
image = img.imread(img_path)
...
file_path = f"{plots_dir}/Normalized_Health_Benefit.csv"
df = pd.read_csv(file_path)
...
img_path = f"{plots_dir}/Annual Health Benefits only mortality.png"
image = img.imread(img_path)
...
img_path = f"{plots_dir}/Monetized Annual Health Benefits (million $).png"
image = img.imread(img_path)
...
file_path = f"{plots_dir}/Monetized_Health_Benefit.csv"
df = pd.read_csv(file_path)
...
img_path = f"{plots_dir}/Monetized Annual Health Benefits (million $) only mortality.png"
image = img.imread(img_path)
...
supp_file = os.path.join(plots_dir, "supplementary.txt") 
